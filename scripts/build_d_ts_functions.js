  ...
  files.forEach(file => {
    processWrapperFile(file, input => {
      ...
      if(json.type === 'function'){
        ...
        return chunk.slice(0, -3) + `,${whitespace}"typedef": "declare function ${json.name}(${json.params?.map(p => p[0]+': '+getParam(p[1]))?.join(', ') ?? ''}): ${json.return_object ?? getParam(json.return?.[0])}"` + chunk.slice(-2);
      }
      return input;
    });
  });

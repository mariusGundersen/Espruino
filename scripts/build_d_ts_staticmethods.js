...
getWrapperFiles(files => {
  files.forEach(file => {
    processWrapperFile(file, input => {
      ...
      if(json.type === 'staticmethod'){
        var whitespace = /^(\s+)\S/g.exec(chunk)[1];
        return chunk.slice(0, -3) + `,${whitespace}"typedef": "static ${json.name}(${json.params?.map(p => p[0]+': '+getParam(p[1]))?.join(', ') ?? ''}): ${json.return_object ?? getParam(json.return?.[0])}"` + chunk.slice(-2);
      }
      return input;
    });
  });
